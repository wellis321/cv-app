...
export function buildDocDefinition({ cvData, profile, config, cvUrl, qrCodeImage, templateId }) {
    ...
    const footer = (currentPage, pageCount) => {
        ...
        if (config?.showFreePlanBranding && config?.siteUrl) {
            ...
            items.push({ text: [{ text: `Simple CV Builder created by William Ellis. Dedicated to helping you get the job you want. © ${year} ` }, { text: 'simple-cv-builder.com', link: config.siteUrl }], alignment: 'center', fontSize: 7, color: mutedColor, margin: [0, 4, 0, 0] })
        }
