...
export async function createCvDocDefinition(
    ...
    if (config.includeQRCode && cvUrl) {
        try {
            qrCodeImage = await QRCode.toDataURL(cvUrl, {
                width: 200,
                margin: 2,
    ...
    if (config.sections.profile) {
        ...
        // Build header columns with photo and/or QR code
        ...
        // Add QR code if enabled and generated
        if (qrCodeImage) {
            headerImages.push({
                width: 'auto',
                image: qrCodeImage,
        ...
        // Create header with or without images
        if (headerImages.length > 0) {
            ...
            // No photo or QR code, just add the text
            content.push({
                stack: headerContent
            });
        }
...
export async function generateCvPdf(
    ...
    const docDefinition = await createCvDocDefinition(cvData, config, cvUrl);
