 ...
 */
export async function ensureBucketExists(bucket: string): Promise<{ exists: boolean; error?: string }> {
    try {
        ...
        try {
            ...
            if (createError) {
                ...
                if (createError.message?.includes('row-level security policy')) {
                    console.warn(`RLS policy prevented bucket creation for "${bucket}", but it might still be usable`);
                    return { exists: true, error: `Note: RLS prevented modification: ${createError.message}` };
                }
            ...
            return { exists: bucketExists, error: createErr instanceof Error ? createErr.message : String(createErr) };
        }
...
export async function fileExists(bucket: string, path: string): Promise<boolean> {
    try {
        ...
        const { data, error } = await supabase.storage
            .from(bucket)
            .createSignedUrl(path, 1); // Just 1 second expiry to check existence
 ...
 * @returns Status and any error message
 */
export async function ensurePublicAccess(bucket: string): Promise<{ success: boolean; error?: string }> {
    try {
