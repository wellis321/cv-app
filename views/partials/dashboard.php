...
<div class="py-6">
    ...
    <?php
    // Auto-generated usernames (e.g. "user1a2b3c4d") aren't great for a public CV link -
    // nudge the user to pick their own. This stays until they change it (see profile.php).
    $dashboardUsername = db()->fetchOne("SELECT username FROM profiles WHERE id = ?", [$userId])['username'] ?? '';
    $hasAutoGeneratedUsername = preg_match('/^user[a-z0-9]{8}$/i', $dashboardUsername);
    ?>
    <?php if ($hasAutoGeneratedUsername): ?>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
            <div class="rounded-md bg-blue-50 border border-blue-200 p-4 flex items-center justify-between flex-wrap gap-3">
                <div class="flex items-center">
                    <svg class="h-5 w-5 text-blue-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
                    </svg>
                    <p class="ml-3 text-sm text-blue-800">Your CV link is using an auto-generated username. Pick your own so it's easier to share and remember.</p>
                </div>
                <a href="/profile.php" class="text-sm font-medium text-blue-700 hover:text-blue-900 underline whitespace-nowrap">Set your username &rarr;</a>
            </div>
        </div>
    <?php endif; ?>
