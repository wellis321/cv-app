...
<div class="p-6 max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form id="rewrite-form" class="space-y-6" <?php echo $ai_scope_limited ? ' data-ai-scope-limited="1"' : ''; ?> data-job-id="<?php echo e($preselectJobId ?? ''); ?>">
            ...
            <!-- One-click full CV (matches jobs view behavior when a job is selected) -->
            <div id="one-click-full-cv-block" class="bg-indigo-50 border border-indigo-200 rounded-lg p-4 mb-4 hidden">
                <p class="text-sm font-medium text-indigo-800 mb-2">Same as &quot;Generate AI CV for this job&quot;</p>
                <p class="text-xs text-indigo-700 mb-3">One-click tailored CV using default sections (summary, work experience, skills) — identical to the job view.</p>
                <button type="button" id="one-click-generate-btn" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                    Generate full CV (one-click)
                </button>
            </div>
...
(function() {
    ...
    // One-click full CV block: show when a job is selected (matches jobs view payload)
    const oneClickBlock = document.getElementById('one-click-full-cv-block');
    const oneClickBtn = document.getElementById('one-click-generate-btn');
    function updateOneClickVisibility() {
        const sel = document.getElementById('job_application_id');
        if (oneClickBlock && sel) {
            oneClickBlock.classList.toggle('hidden', !sel.value);
        }
    }
    if (jobApplicationSelect) {
        jobApplicationSelect.addEventListener('change', updateOneClickVisibility);
    }
    updateOneClickVisibility();

    // One-click generate: send job_application_id + csrf + variant_name only (same as jobs view)
    if (oneClickBtn) {
        oneClickBtn.addEventListener('click', async function() {
            const jobId = document.getElementById('job_application_id')?.value;
            if (!jobId) {
                alert('Please select a job application first.');
                return;
            }
            if (!confirm('This will generate a new AI-rewritten CV variant for this job (same as "Generate AI CV for this job"). Continue?')) return;
            const fd = new FormData();
            fd.append('<?php echo CSRF_TOKEN_NAME; ?>', document.querySelector('input[name="<?php echo CSRF_TOKEN_NAME; ?>"]')?.value || '');
            fd.append('job_application_id', jobId);
            fd.append('variant_name', document.getElementById('variant_name')?.value || 'AI-Generated CV');
            oneClickBtn.disabled = true;
            oneClickBtn.innerHTML = '<svg class="animate-spin h-4 w-4 inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Generating...';
            try {
                const r = await fetch('/api/ai-rewrite-cv.php', { method: 'POST', body: fd, credentials: 'include' });
                ...
                oneClickBtn.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg> Generate full CV (one-click)';
            }
        });
    }
